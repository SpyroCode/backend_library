...
router.post("/", async (req, res) => {
  ...
  if (name && email && password && role && status) {
    ...
    logger.error(
      `[CREATE USER] No found data, name : ${name} ,email: ${email}, password: ${password},role : ${role},status:${status}`
    );
...
router.post("/update/:id", async (req, res) => {
  ...
  logger.info(JSON.stringify({ name, email }));
  if (name && email && password) {
    ...
    logger.error(
      `[UPDATE USER] No found data, name : ${name} ,email: ${email}, password: ${password}`
    );
    const message = "Faltan datos";
    res.status(500).send({ message });
    res.end();
